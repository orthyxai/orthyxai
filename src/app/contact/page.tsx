    ...
    <div className="py-24 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-5xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <AnimatedSection>
            <Typography variant="h2" className="text-5xl font-extrabold mb-4 text-gray-900">
              {t.contact.title}
            </Typography>
            <Typography variant="body1" className="text-xl text-gray-600 mb-10">
              {t.contact.subtitle}
            </Typography>
            <div className="space-y-6">
              <Paper className="p-6 bg-white/80 border-gray-200 border rounded-2xl flex items-center gap-4" elevation={0}>
                <div className="p-3 rounded-xl bg-primary/10 text-primary"><LocationOn /></div>
                <div><div className="font-bold text-gray-900">{t.contact.address}</div><div className="text-gray-600">{t.contact.address_val}</div></div>
              </Paper>
              <Paper className="p-6 bg-white/80 border-gray-200 border rounded-2xl flex items-center gap-4" elevation={0}>
                <div className="p-3 rounded-xl bg-primary/10 text-primary"><Phone /></div>
                <div><div className="font-bold text-gray-900">{t.contact.phone}</div><div className="text-gray-600">{t.contact.phone_val}</div></div>
              </Paper>
              <Paper className="p-6 bg-white/80 border-gray-200 border rounded-2xl flex items-center gap-4" elevation={0}>
                <div className="p-3 rounded-xl bg-primary/10 text-primary"><Email /></div>
                <div><div className="font-bold text-gray-900">{t.contact.email_label}</div><div className="text-gray-600">{t.contact.email_val}</div></div>
              </Paper>
            </div>
          </AnimatedSection>

          <AnimatedSection delay={0.2}>
            <Paper className="p-8 bg-white/80 border-gray-200 border rounded-3xl" elevation={0}>
              <Typography variant="h5" className="font-bold mb-6 text-gray-900">{t.contact.send}</Typography>
